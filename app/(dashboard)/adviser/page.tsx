...
export default function AdviserDashboard() {
  const { user } = useAuth();
  const lastName = user?.profile?.lastName || 'Adviser';
  ...
  const sampleQuestions = [
    {
      ...
      description:
        'Quickly look up handbook policies, defense procedures, and program requirements when advising your students — every answer is cited.',
      questions: [
        'What is the maximum residency for M.S. Environmental Science?',
        'What are the requirements to advance to thesis defense?',
        'What is the procedure for changing a student’s adviser?',
    ...
    },
    {
      ...
      description:
        'Search the Journal of Environmental Science and Management archive for prior work, methodologies, and citations relevant to your advisees’ research.',
      questions: [
        'Recent JESAM studies on watershed management methodologies',
        'JESAM papers using remote sensing in agroforestry',
        'Compare sampling approaches for soil microbiome studies',
        'Find JESAM articles on climate adaptation in Philippine agriculture',
      ],
    },
  ];
  ...
  const workflowSteps = [
    ...
    },
    {
      ...
      desc: 'Get an authoritative response with inline citations and a References section — ready to share with advisees.',
    },
  ];
      ...
      <div className="mb-6 relative overflow-hidden rounded-2xl bg-[#0C0B5D] p-5 sm:p-6 text-white shadow-lg">
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h2 className="mt-1 text-xl sm:text-2xl font-bold tracking-tight">Welcome back, Dr. {lastName}</h2>
            <p className="mt-1 text-sm text-white/80 max-w-2xl">
              Your AI reference companion for SESAM policy and JESAM research — cited answers
              you can confidently share with your advisees.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Need a quick reference?</h1>
              <p className="mt-1 text-sm text-gray-600 max-w-xl">
                Look up a SESAM policy, defense requirement, or JESAM paper without leaving your
                desk — SINAG cites every source.
              </p>
            </div>
          </div>
          <Link
          ...
          <div key={category} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <ul className="space-y-2">
              {questions.map((q) => (
                <li key={q}>
                  <Link
                    href={`/adviser/ai-chat?q=${encodeURIComponent(q)}`}
        ...
        <div className="mt-8 grid gap-3 border-t border-gray-100 pt-6 sm:grid-cols-3">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">Reference policy and literature without context-switching.</p>
            </div>
          </div>
        </div>
      </section>
